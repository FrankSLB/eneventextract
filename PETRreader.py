...
def read_story_input(rawstory, storytitle, storydate, storysrc, storyurl, story_id):
    """read a story to a dic"""

    holding = {}

    tmp_list = []
    try:
        tmp_list = rawstory.strip().replace("\r\n", '\n').replace('\n', ' ').split('.')
    except UnicodeDecodeError:
        sys.exit("The article contains non-English characters")
    sentences = ['{}.'.format(item.strip()) for item in tmp_list if item.strip() != ""]
    # get the front 7 sentences
    if len(sentences) > 7:
        sentences = sentences[:7]

    if sentences:
        nlp = StanfordCoreNLP('stanford-corenlp-full-2017-06-09')

        #story_id = str(hash(storyurl))
        meta_dict = {'title': storytitle, 'date': storydate, 'source': storysrc, 'url': storyurl}

        #t0 = time.time()
        sents_dict = {}
        for index, text in enumerate(sentences):
            try:
                parse = utilities._format_parsed_str(nlp.parse(text))
            except Exception as e:
                continue

            sent_dict = {'content': text, 'parsed': parse}

            sent_id = index
            sents_dict[sent_id] = sent_dict
        #used = time.time() - t0

        story_dict = {}
        story_dict["sents"] = sents_dict
        story_dict['meta'] = meta_dict

        holding[story_id] = story_dict
